...
private:
  ...
  void timerCallback()
  {
    ...
    size_t seq = publish_number_++;
    image_tools::ROSCvMatContainer container(frame, header, seq);

    // RCLCPP_INFO(get_logger(), "Received image #%zu", container.seq());

    // Publish the image message and increment the publish_number_.
    RCLCPP_INFO(get_logger(), "Publishing image #%zu", seq);
